...
class Plan:
    ...
    @property
    def _snapshot_mapping(self) -> t.Dict[SnapshotId, Snapshot]:
    ...
    def _dag(self) -> DAG[SnapshotId]:
        if self.__dag is None:
            self.__dag = DAG()
            for snapshot in self._snapshot_mapping.values():
    ...
    def set_choice(self, snapshot: Snapshot, choice: SnapshotChangeCategory) -> None:
        ...
        for child_s_id in self.indirectly_modified[snapshot.snapshot_id]:
            child_snapshot = self._snapshot_mapping[child_s_id]
    ...
    def _add_restatements(self) -> None:
        ...
        for model_fqn in restate_models:
            ...
            for downstream_s_id in self._dag.downstream(snapshot.snapshot_id):
                if is_restateable_snapshot(self._snapshot_mapping[downstream_s_id]):
        ...
        for s_id in self._dag:
            if s_id not in restatements:
                continue
            snapshot = self._snapshot_mapping[s_id]
    ...
    def _build_directly_and_indirectly_modified(self) -> t.Tuple[t.List[Snapshot], SnapshotMapping]:
        ...
        for s_id, snapshot in self._snapshot_mapping.items():
            if s_id in self.context_diff.modified_snapshots:
                if self.context_diff.directly_modified(s_id):
    ...
    def _categorize_snapshots(self) -> None:
        ...
        for s_id in self._dag:
            snapshot = self._snapshot_mapping.get(s_id)
            if not snapshot or snapshot.change_category:
                continue

            if s_id in self.context_diff.modified_snapshots:
                is_directly_modified = self.context_diff.directly_modified(s_id)
                if self.is_new_snapshot(snapshot):
                    if self.forward_only:
                        ...
                        s_id_with_missing_column: t.Optional[SnapshotId] = None
                        this_sid_with_downstream = self.indirectly_modified.get(s_id, set()) | {
                            s_id
                        }
                        for downstream_s_id in this_sid_with_downstream:
                            downstream_snapshot = self._snapshot_mapping[downstream_s_id]
                            if (
                                downstream_snapshot.is_model
                                and downstream_snapshot.model.columns_to_types is None
                            ):
                                s_id_with_missing_column = downstream_s_id
                                break

                        new, old = self.context_diff.modified_snapshots[s_id]
                        if s_id_with_missing_column is None:
    ...
    def _ensure_no_forward_only_revert(self) -> None:
        ...
        """
        for s_id, (candidate, promoted) in self.context_diff.modified_snapshots.items():
            if (
                ...
                raise PlanError(
                    f"Detected an existing version of model '{s_id.name}' that has been previously superseded by a forward-only change. "
    ...
    def _ensure_no_broken_references(self) -> None:
        for snapshot in self.context_diff.snapshots.values():
            ...
            if broken_references:
                raise PlanError(
                    f"""Removed {', '.join(f"'{x.name}'" for x in broken_references)} are referenced in '{snapshot.name}'. Please remove broken references before proceeding."""
                )
